...
class VllmConfig:
    ...
    def validate_nvfp4_kv_cache_with_mla(self) -> "VllmConfig":
        if self.model_config is None:
            return self
        if (
            self.cache_config.cache_dtype in ("nvfp4", "fp8_k_nvfp4_v")
            and self.model_config.use_mla
        ):
            raise ValueError(
                f"{self.cache_config.cache_dtype} KV cache is not supported with "
                "MLA (Multi-head Latent Attention) backends. Please use a "
                "different --kv-cache-dtype "
