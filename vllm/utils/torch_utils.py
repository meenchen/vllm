...
def fp8_k_nvfp4_v_cache_split_views(
    ...
    if kv_cache.stride(-1) != 1 or any(
        stride % total_dim for stride in kv_cache.stride()[1:3]
    ):
        raise ValueError(
            "fp8_k_nvfp4_v cache requires a contiguous last dimension and "
            "NHD- or HND-compatible inner strides"
        )
