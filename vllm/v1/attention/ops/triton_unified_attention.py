...
def _cast_kv_tile(data, Q, tensor_scale, KV_QUANT_MODE: tl.constexpr):
    ...
    - ``KV_QUANT_MODE in {1, 5}`` (FP8 per-tensor K): dequantize using the
      tensor-wide scale, unless Q is also FP8 and the caller folds the scales
      into the attention score.
    """
    if KV_QUANT_MODE == 1 or KV_QUANT_MODE == 5:
        if Q.dtype.is_fp8():
...
def kernel_unified_attention(
    ...
    # Swizzled NVFP4 V block scales, used iff KV_QUANT_MODE == 5.
    ...
    # FP8_PER_TOKEN_HEAD (3), FP8_K_NVFP4_V (5).
    ...
    USE_MIXED_KV: tl.constexpr = KV_QUANT_MODE == 5
